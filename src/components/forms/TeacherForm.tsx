...
const schema = z.object({
  ...
  address: z.string().min(1, { message: "Address is required!" }),
  bloodType: z.string().min(1, { message: "Blood Type is required!" }),
  birthday: z.string().min(1, { message: "Birthday is required!" }), // Store as string for compatibility
  sex: z.enum(["male", "female"], { message: "Sex is required!" }),
  img: z.instanceof(File).optional(), // Image is optional
});
  ...
  const onSubmit = async (formData: Inputs) => {
    try {
      ...
      Object.keys(formData).forEach((key) => {
        const value = formData[key as keyof Inputs];
      ...
      const response = await fetch("http://10.13.40.144:7001/api/teachers/add", {
        method: "POST",
        body,
      });
  ...
  return (
    <form className="flex flex-col gap-8" onSubmit={handleSubmit(onSubmit)}>
      ...
      <button
        type="submit"
        disabled={loading}
        className={`bg-blue-400 text-white p-2 rounded-md ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
      >
